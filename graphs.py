...
if __name__ == "__main__":
    ...
    data = get_thar_shifts("opt_data_esp_stabil.txt")
    ...
    with plt.style.context(["retro", "grid"]):
        ...
        ax.scatter(
            data["mjd"], data["y_shift"], label="Dispersion axis", color="crimson"
        )
        ...
        slope, intercept, r_value, p_value, std_err = stats.linregress(
            data["mjd"][good_data], data["x_shift"][good_data]
        )
        plt.plot(
            data["mjd"][not_so_good_data],
            slope * data["mjd"][not_so_good_data] + intercept,
            "--",
            color="black",
        ...
        plt.scatter(
            data["mjd"][not_so_good_data],
            data["x_shift"][not_so_good_data],
            color="black",
            label="Not calm state",
        )
        plt.scatter(
            data["mjd"][good_data],
            data["x_shift"][good_data],
            color="#009E73",
            label="Calm state",
        )  # Line part
        ...
        ax.axvspan(
            data["mjd"][not_so_good_data][0] - 0.01,
            data["mjd"][good_data][0],
            facecolor="yellow",
            alpha=0.5,
        )
        ax.axvspan(
            data["mjd"][not_so_good_data][-1] + 0.025,
            data["mjd"][good_data][-1],
            facecolor="yellow",
            alpha=0.5,
        )
        ax.xaxis.set_major_formatter(ticker.FormatStrFormatter("%0.1f"))
        ...
        slope_y, intercept_y, r_value_y, p_value_y, std_err_y = stats.linregress(
            data["mjd"][good_data], data["y_shift"][good_data]
        )

        # Regression part
        plt.plot(
            data["mjd"][not_so_good_data],
            slope_x * data["mjd"][not_so_good_data] + intercept_x,
            "--",
            color="#1e8cb0",
        ...
        plt.plot(
            data["mjd"][not_so_good_data],
            slope_y * data["mjd"][not_so_good_data] + intercept_y,
            "--",
            color="#e953da",
            label="Dispersion regression",
            linewidth=2,
        )
